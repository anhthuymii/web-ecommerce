...
const ListConfirm = ({ onTotalOrders }) => {
  ...
  const fetchOrders = async () => {
    try {
      let apiUrl = `${process.env.REACT_APP_API}/api/v1/order/get-orders?page=${page}&limit=${limit}&orderStatus=Đã%20xác%20nhận`;
      if (status.length > 0) {
  ...
  return (
    <div className="w-full min-h-screen flex flex-row">
      <Title title={"Đơn hàng đã xác nhận - JEANO Store"} />
      <Sidebar />
      <div className="flex flex-col space-y-6 py-1 px-2">
        ...
        {orders.length === 0 ? (
          <p className="text-lg font-bold">Không có đơn hàng đã xác nhận</p>
        ) : (
          <div>
