...
const ListCancel = ({ onTotalOrders }) => {
  ...
  const fetchOrders = async () => {
    try {
      let apiUrl = `${process.env.REACT_APP_API}/api/v1/order/get-orders?page=${page}&limit=${limit}&orderStatus=Đã%20hủy%20đơn`;
      if (status.length > 0) {
  ...
  return (
    <div className="w-full min-h-screen flex flex-row">
      <Title title={"Đơn hàng đã hủy - JEANO Store"} />
      <Sidebar />
      <div className="flex flex-col space-y-6 py-1 px-2">
        <div>
          <Breadcrumbs title="Đơn hàng đã hủy" prevLocation={prevLocation} />
        </div>
        <div>
          <ListOrder />
        </div>
