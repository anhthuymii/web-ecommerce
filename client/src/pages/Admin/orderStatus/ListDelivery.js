...
const ListDelivery = ({ onTotalOrders }) => {
  ...
  const fetchOrders = async () => {
    try {
      let apiUrl = `${process.env.REACT_APP_API}/api/v1/order/get-orders?page=${page}&limit=${limit}&orderStatus=Đang%20giao%20hàng`;
      if (status.length > 0) {
  ...
  return (
    <div className="w-full min-h-screen flex flex-row">
      ...
      <div className="flex flex-col space-y-6 py-1 px-2">
        <div>
          <Breadcrumbs title="Đơn hàng đang giao" prevLocation={prevLocation} />
        </div>
        <div>
          <ListOrder />
        </div>
