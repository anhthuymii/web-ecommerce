...
const GetReviews = () => {
    ...
    const fetchReviews = async () => {
      try {
        const { data } = await axios.get(
          `${process.env.REACT_APP_API}/api/v1/review/get-all-reviews?page=${page}&limit=${limit}`
        );
  ...
  return (
    <div className="w-full min-h-screen flex flex-col md:flex-row">
      <Title title={"Quản lý đánh giá sản phẩm - JEANO Store"} />
      <Sidebar />
      <div className="flex flex-col space-y-6 py-1 px-2 w-full md:w-4/5">
        ...
        <div>
          <CountReview onFilterChange={handleFilterChange} />
        </div>
          ...
          {loading ? (
            ...
            <div className="justify-center items-center grow p-5 border rounded">
              {reviews
                .filter((review) => {
                  if (filter === "good") {
                    return review.star >= 4 && review.star <= 5;
                  } else if (filter === "bad") {
                  ...
                  <div className="flex justify-center">
                    <div className="w-full h-auto">
                      <article className="shadow-md rounded-md m-3 p-5">
                        ...
                        {review.orders.map((order) => (
                          <div
                            className="right-0 flex flex-end"
                            key={order.order}
                          >
                            {order.products.map((product) => (
                              <div className="flex flex-row" key={product._id}>
                                <div
                                  className="underline flex flex-end"
                                  onClick={() =>
                                ...
                                <div
                                  ...
                                  className="mx-5 text-red-500 underline flex flex-end"
                                >
                                  Xóa đánh giá
                                </div>
                              </div>
