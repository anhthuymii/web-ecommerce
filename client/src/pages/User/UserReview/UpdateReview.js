...
const UpdateReview = ({ reviewId, onCancel, onReviewSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `${process.env.REACT_APP_API}/api/v1/review/update-product-review/${reviewId}`,
        formData,
        {
          headers: {
      ...
      if (data?.success) {
        ...
        toast.error(data.message || "Có lỗi xảy ra khi chỉnh sửa đánh giá");
      }
    } catch (error) {
      console.error(error);
      toast.error("Có lỗi xảy ra khi chỉnh sửa đánh giá");
    }
  };
