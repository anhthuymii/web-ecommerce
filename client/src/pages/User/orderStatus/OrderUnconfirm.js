...
const OrderUnconfirm = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      }/api/v1/order/get-user-order/${userId}?orderStatus=${encodeURIComponent(
        "Chờ xác nhận"
      )}`;
  ...
  return (
    <div className="w-full min-h-screen flex flex-col md:flex-row">
      <Title title={"Đơn hàng chưa xác nhận - JEANO Store"} />
      <SidebarUser />
      <div className="flex flex-col space-y-6 py-1 px-2 w-full md:w-4/5">
        <div>
          <Breadcrumbs title="Đơn hàng" prevLocation={prevLocation} />
        </div>
        <div>
          <OrderSteps />
        </div>
                    ...
                    {firstProduct ? (
                      <li>
                        <img
                          ...
                          src={`${process.env.REACT_APP_API}/api/v1/product/product-photo/${firstProduct._id}`}
                  ...
                  <div className="w-full ">
                    <div className="flex flex-row justify-end">
                      {order.orderStatus === "Chờ xác nhận" && (
                        <p
                          className="hover:underline text-base font-semibold mx-5 cursor-pointer text-red-500"
